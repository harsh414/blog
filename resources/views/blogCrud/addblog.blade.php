...
@section('content')
    ...
    <form action="{{route('blog.store')}}" method="POST" enctype="multipart/form-data" class="px-3 py-6">
        {{@csrf_field()}}
        <div>
            <input type="text" name="title" id="title" class="bg-white text-sm border-none w-full text-sm rounded mt-4 placeholder-gray-500" placeholder="Your Blog title goes here..:)" >
        </div>
        <div class="mt-12">
            <textarea name="blog-description" id="blog-description" rows="16" class="w-full z-10 bg-gray-100 rounded-xl border-none placeholder-gray-900 text-sm px-4 py-2" placeholder="Share your Blog here :)"></textarea>
        </div>
        <div class="flex items-center justify-between space-x-3 mt-2">
            <button type="submit" class="flex items-center justify-center w-1/2 md:w-1/6 h-9 text-xs bg-black text-white font-semibold rounded-xl border border-blue hover:bg-blue-hover transition duration-150 ease-in px-6 py-3">
